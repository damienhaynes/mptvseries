	...
	public class DBEpisode : DBTable, ICacheable<DBEpisode>
	{
		...
		public bool HasMediaInfo
		{
			get
			{
				...
				if (String.IsNullOrEmpty(this["localPlaytime"]))
					...
					int noAttempts = 0;
					if (!int.TryParse(this["localPlaytime"], out noAttempts))
						return true;
					...
					if (noAttempts >= -MAX_MEDIAINFO_RETRIES)
						return false;
		...
		public static void GetSeasonEpisodeCounts(DBSeason season, out int epsTotal, out int epsUnWatched)
		{
			...
			for (int i = 0; i < results.Rows.Count; i++) {
				...
				Match match = r.Match(results.Rows[i][2].ToString());
				DateTime firstAired;
		...
		{
			if (this.m_onlineEpisode != null)
				return m_onlineEpisode.ToString();
