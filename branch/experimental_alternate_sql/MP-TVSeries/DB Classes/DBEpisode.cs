...
namespace WindowPlugins.GUITVSeries
{
    ...
    public class DBEpisode : DBTable, ICacheable<DBEpisode>
    {
            ...
            : base(cTableName)
        {
            ...
            DBTVSeries.CreateDBIndices("create index if not exists epComp1 ON local_episodes(CompositeID ASC)","local_episodes","epComp1",false);
            DBTVSeries.CreateDBIndices("create index if not exists epComp2 ON local_episodes(CompositeID2 ASC)","local_episodes","epComp2",true);
        ...
        static List<DBEpisode> GetFirstUnwatched(SQLCondition conditions)
        {
            ...
            if (results.Rows.Count > 0)
            {
                for (int index = 0; index < results.Rows.Count; index++)
                {
					...
					episode.m_onlineEpisode.Read(results, index);
                    outList.Add(episode);
                }
            }
            return outList;
