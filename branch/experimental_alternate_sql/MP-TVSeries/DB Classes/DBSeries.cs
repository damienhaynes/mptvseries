...
namespace WindowPlugins.GUITVSeries
{
    ...
    public class DBSeries : DBTable
    {
            ...
            : base(cTableName)
        {
            ...
            DBTVSeries.CreateDBIndices("create index if not exists seriesIDLocal on local_series(ID ASC)", "local_series", "seriesIDLocal", true);
        }
        ...
        private static List<DBSeries> Get(String sqlQuery)
        {
            ...
            if (results.Rows.Count > 0)
            {
                for (int index = 0; index < results.Rows.Count; index++)
                {
                    ...
                    series.Read(results, index);
                    series.m_onlineSeries = new DBOnlineSeries();
                    series.m_onlineSeries.Read(results, index);
                    outList.Add(series);
