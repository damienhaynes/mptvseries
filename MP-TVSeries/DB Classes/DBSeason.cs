            ...
            get
            {
                switch (fieldName)
                {
                    case DBSeason.cEpisodeCount: {
                            SQLiteResultSet results = DBTVSeries.Execute(EpisodeCountQuery(this[cSeriesID], this[cIndex], false));
                    ...
                    case DBSeason.cEpisodesUnWatched: {
                            SQLiteResultSet results = DBTVSeries.Execute(EpisodeCountQuery(this[cSeriesID], this[cIndex], true));
                            if (results.Rows.Count > 0) {
                                return new DBValue(DatabaseUtility.GetAsInt(results, 0, "count"));
                            } else {
                                return new DBValue(0);
